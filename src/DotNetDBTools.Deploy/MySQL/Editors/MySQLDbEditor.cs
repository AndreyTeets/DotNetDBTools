        ...
        public override void PopulateDNDBTSysTables(Database database)
        {
            ...
            foreach (MySQLTable table in db.Tables)
            {
                ...
                foreach (Column c in table.Columns)
                    QueryExecutor.Execute(new MySQLInsertDNDBTSysInfoQuery(c.ID, table.ID, DbObjectsTypes.Column, c.Name, c.GetExtraInfo()));
                ...
                foreach (CheckConstraint ck in table.CheckConstraints)
                    QueryExecutor.Execute(new MySQLInsertDNDBTSysInfoQuery(ck.ID, table.ID, DbObjectsTypes.CheckConstraint, ck.Name, ck.GetExtraInfo()));
