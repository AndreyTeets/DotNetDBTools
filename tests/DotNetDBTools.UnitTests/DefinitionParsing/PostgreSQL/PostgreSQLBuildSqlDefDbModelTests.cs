            ...
            new PostgreSQLTable()
            {
                ...
                CheckConstraints= new()
                {
                    new CheckConstraint()
                    {
                        ...
                        CodePiece = new CodePiece() { Code = "CHECK (Col2 != 'Col2 DECIMAL(6, 1) NOT NULL DEFAULT 7.36,')" },
                    },
                    new CheckConstraint()
                    {
                        ID = new Guid("A736AE77-B7E4-40C3-824F-BD20DC270A14"),
                        Name = null,
                        CodePiece = new CodePiece() { Code = "CHECK (Col4 = 'CONSTRAINT CK_String_Check2 CHECK ( Col3 >= 0 ),' AND f1(f2())=' quo''te g1(g2(g3)))' AND TRUE)" },
                    },
                    new CheckConstraint()
                    {
                        ID = new Guid("A836AE77-B7E4-40C3-824F-BD20DC270A14"),
                        Name = "CK_Table1_Check3",
                        CodePiece = new CodePiece() { Code = @"CHECK (""Col3"" >= 0)" },
                    },
                },
                ForeignKeys= new()
                {
